...
const VerdadeOuDesafio: React.FC<Props> = ({ config, onExit }) => {
  const [state, setState] = useState<TODState>(() => initGame(config));
  ...
  const wrap = (children: React.ReactNode) => (
    <div className="page-wrapper flex flex-col p-5">
      <GameHeader title="Verdade ou Desafio" round={state.turnsPlayed + 1} totalRounds={state.totalTurns} onExit={onExit} />
