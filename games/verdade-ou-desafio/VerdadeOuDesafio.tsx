...
const VerdadeOuDesafio: React.FC<Props> = ({ config, onExit, online, roomCode, playerId, isHost }) => {
  ...
  const wrap = (children: React.ReactNode, header = true) => (
    <div className="page-wrapper flex flex-col p-5">
      {header && state && <GameHeader title="Verdade ou Desafio" round={state.turnsPlayed + 1} totalRounds={state.totalTurns} onExit={!online || isHost ? onExit : undefined} onRestartRound={online && isHost ? resetRound : undefined} onRestartGame={online && isHost ? reset : undefined} />}
